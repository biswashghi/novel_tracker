...
grep -Fq 'install-keycloak-apple-provider.sh' "$ROOT_DIR/scripts/deploy-vps.sh"
grep -Fq 'install-keycloak-apple-provider.sh' "$ROOT_DIR/scripts/remote/deploy-production.sh"
grep -Fq 'apple-identity-provider-1.14.0.jar:/opt/keycloak/providers/apple-identity-provider-1.14.0.jar:ro' "$ROOT_DIR/compose.production.yml"
